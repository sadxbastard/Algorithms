...
public:
	...
	void PrintRuntime(const int RatioStep, const int Trials);
	...
	void Saw(int Id, const int _Size) const;
...
template <typename Type>
void InitializerAndHandlerArray<Type>::PrintRuntime(const int RatioStep, const int Trials) {
	...
	for (int N = Step; N <= Size; N += Step)
	{
		...
		for (int i = 0; i < Trials; i++)
		{
			...
			//qsort(pointer, Size, sizeof(Array[0]), compare);
			t2 = GetTickCount();
			Time += (t2 - t1);
		}
		cout << N << " " << Time / Trials << endl;
		cout << "Array sorted: " << (CheckOrder(N) ? "Yes" : "No") << endl;
		cout << "Counter operations: " << counter << endl << endl;
		//PrintArray(N);
	}
...
template <typename Type>
void InitializerAndHandlerArray<Type>::Saw(int Id, const int _Size) const
{
...
void InitializerAndHandlerArray<Type>::ModifiedBubble(int Id, const int _Size) {
	...
	while (Low < _Size - 1) {
		for (IndexOfChanged = Id = Size - 1; Id > Low; Id--) {
			counter++;
...
int main() {
	InitializerAndHandlerArray<double> Array(3000, 0, 0, 10); // (Size, Max, Min, Intervals) // 3574 без обертки
	Array.PrintRuntime(1, 10); // (RatioStep, Trials)
	return 0;
}